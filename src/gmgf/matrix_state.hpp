...
namespace gmgf {
   ...
   * \brief 距離間距離が格納された行列を利用して，
   頂点間距離の計算の高速化を図った状態．
   */
  class matrix_state : public basic_state {
  protected:
    /** \brief 最小グラフの頂点間距離を表す行列 */
    igraph_matrix_t d_min;
    /** \brief 最小グラフの最短経路数を表す行列 */
  ...
  public:
     ...
     * \param s_min 最小グラフの最短経路数を表す行列
     * \param d_max 最大グラフの頂点間距離を表す行列
     * \param s_max 最大グラフの最短経路数を表す行列
     */
    matrix_state(graph_config* config, igraph_t G,
     ...
     *
     * オブジェクトのコピーを作成する．
     * gmgf::matrix_state::matrix_state
     (graph_config*, igraph_t, igraph_matrix_t, igraph_matrix_t,
     igraph_matrix_t, igraph_matrix_t)
