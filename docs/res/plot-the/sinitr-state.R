...
plot_height <- 2.5
cairo_pdf('sinitr-state.pdf', width = 3.6, height = plot_height)
pf_state(3)
...
# sinitr-state
cairo_pdf('sinitr-state-lpad.pdf',
          width = convertWidth(sum(p_state_legend$widths) - ylab_width - unit(2, 'mm'), 'inches', TRUE), height = plot_height)
grid.draw(blank)
dev.off()
cairo_pdf('sinitr-state-ylab.pdf', width = convertWidth(ylab_width + unit(2, 'mm'), 'inches', TRUE), height = plot_height)
grid.draw(p_state_ylab)
dev.off()
cairo_pdf('sinitr-state-d3-yaxis.pdf', width = convertWidth(yaxis_width, 'inches', TRUE), height = plot_height)
grid.draw(p_state_d3_yaxis)
dev.off()
cairo_pdf('sinitr-state-d3.pdf', width = convertWidth(panel_width, 'inches', TRUE), height = plot_height)
grid.draw(p_state_d3)
dev.off()
cairo_pdf('sinitr-state-d4-yaxis.pdf', width = convertWidth(yaxis_width, 'inches', TRUE), height = plot_height)
grid.draw(p_state_d4_yaxis)
dev.off()
cairo_pdf('sinitr-state-d4.pdf', width = convertWidth(panel_width, 'inches', TRUE), height = plot_height)
grid.draw(p_state_d4)
dev.off()
cairo_pdf('sinitr-state-legend.pdf', width = convertWidth(sum(p_state_legend$widths), 'inches', TRUE), height = plot_height)
